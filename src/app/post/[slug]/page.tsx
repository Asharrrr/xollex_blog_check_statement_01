...
export async function generateMetadata({
  ...
  // Next can pass params either as an object or a Promise, so accept both.
  params: { slug: string } | Promise<{ slug: string }>;
}): Promise<Metadata> {
  // Always await params (awaiting a non-promise simply returns it).
  const { slug } = (await params) as { slug: string };

  // debug log (server-side). Check your terminal where Next runs.
  ...
  if (!post) {
    return {
      title: "Post not found | Quollex Blog",
  ...
  const siteUrl =
    ...
    "https://quollex.vercel.app";

  // Ensure imageUrl is always a string (never null)
  const imageUrl =
    post?.mainImage?.asset?._ref && post?.title
      ? `${siteUrl}/api/og?title=${encodeURIComponent(String(post.title))}`
      : `${siteUrl}/og-image.png`;

  return {
    title: String(post.title ?? "Untitled post"),
    description: post.excerpt ?? "Read this article on Quollex Blog.",
    openGraph: {
      title: String(post.title ?? "Untitled post"),
      description: post.excerpt ?? "",
      type: "article",
      publishedTime: post.publishedAt ?? undefined, // ensure undefined, not null
      authors: post.author?.name ? [String(post.author.name)] : undefined,
...
const SinglePostPage = async ({
  ...
  console.log(`[page] rendering post page for slug=${slug}`);
...
// import Container from "@/components/container";
// import OtherPosts from "@/components/otherPosts";
// // import WriteComment from "@/components/write-comment"; // Removed since comments are not used
// import { urlFor } from "@/sanity/lib/image";
// import { getOtherPosts, getPost } from "@/sanity/queries";
// import dayjs from "dayjs";
// import { ChevronLeftIcon } from "lucide-react";
